...
function run_SPINFLUENCE(c,g,nb,penalty,type,filename)
    ...
    if strcmp(filename(1:4),'meme')
        ...
        YvSum=sum(Yv,2);
        Yv=Yv(YvSum>0,:);
        suffix=sprintf('%s_%s_%s_%s_%s_%s_SPINFLUENCE', filename,c,g,nb,penalty,type);
    else 
    ...
    for k=1:nfold
        Itrain = find(Ind~=-1);         % all data used as training index
        Itest  = find(Ind==1);
        Itest=Itest(1);                 % only one data point used as testing
    ...
    dlmwrite(sprintf('/var/tmp/Ypred_%s.mat',paramsIn.filestem),Ypred_ts_val);
    dlmwrite(sprintf('/var/tmp/mu_%s.mat',paramsIn.filestem),rtn);
    dlmwrite(sprintf('/var/tmp/T_%s.mat',paramsIn.filestem),running_times);
    system(sprintf('mv /var/tmp/Ypred_%s.mat ../results/; mv /var/tmp/T_%s.mat ../results/; mv /var/tmp/mu_%s.mat ../results/; rm /var/tmp/%s.log',paramsIn.filestem,paramsIn.filestem,paramsIn.filestem,paramsIn.filestem));
