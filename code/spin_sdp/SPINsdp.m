...
function [rtn,ts_err] =SPINsdp(paramsIn,dataIn,muIn)
...
function [mu_x,Kxx_mu_x,obj,iter] = optimize_x(x,obj,mu_x,Kmu_x,Kxx_mu_x,loss_x,te_x,C,maxiter)
    ...
    while iter < maxiter
        ...
        if Gmax >=G0% max(params.tolerance,G0) % keep current solution
            for u = 1:5
                mu_1(5*(1:size(E,1))-5 + u) = C*(Umax_e == min(u,4))*0.1;
            end
			if sum(mu_1) > 0
...
function [Ymax, YmaxVal,Gmax] = max_gradient_labeling(gradient, max_iter)
    ...
    % greedy approach
    %[Ymax,YmaxVal] = greedy_labeling(gradient);
    [Ymax,YmaxVal] = SDP_labeling_undirected(gradient);
...
function [Ymax, YmaxVal] = greedy_labeling(gradient)
