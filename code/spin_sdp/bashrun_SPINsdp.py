...
def singlerun(filename,c,g,nb,penalty,losstype,node):
        try:
                with open("../results/aT_%s_%s_%s_%s_%s_%s_SPINSDP.mat" % (filename,c,g,nb,penalty,losstype)): pass
        except:
                logging.info(' --> (f)%s,(C)%s,(g)%s,(nb)%s,(p)%s,%s,%s' %( filename,c,g,nb,penalty,losstype,node))
                #print "T_%s_%s_%s_%s_%s_%s_SPINsdp.mat" % (filename,c,g,nb,penalty,losstype)
                os.system(""" ssh -o StrictHostKeyChecking=no %s 'cd /cs/taatto/group/urenzyme/workspace/structured_prediction_network_response/spinsdp/; nohup matlab -nodisplay -nosplash -nodesktop -r "run_SPINsdp '%s' '%s' '%s' '%s' '%s' '%s' " > /var/tmp/tmp_%s_%s_%s_%s_%s_%s_SPINsdp' """ % (node,c,g,nb,penalty,losstype,filename,filename,c,g,nb,penalty,losstype) )
                logging.info(' --| (f)%s,(C)%s,(g)%s,(nb)%s,(p)%s,%s,%s' %( filename,c,g,nb,penalty,losstype,node))
        time.sleep(10)
        pass
